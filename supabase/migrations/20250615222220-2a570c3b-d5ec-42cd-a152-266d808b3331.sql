...
-- Adicionar coluna username na tabela user_profiles
ALTER TABLE public.user_profiles 
ADD COLUMN username TEXT;

-- Criar índice único para username (garantir que cada username seja único)
CREATE UNIQUE INDEX idx_user_profiles_username 
ON public.user_profiles (username) 
WHERE username IS NOT NULL;

-- Atualizar a função de setup do usuário para incluir username
...
BEGIN
  ...
  -- 1. Insert into user_profiles (agora incluindo username)
